...
脚本使用编辑器:nano 用法:\nCtrl O保存 之后提示是否以原文件名保存 Ctrl X退出(用了就清楚了)
  ...
  if type == "1":
    ...
    action = install_command + " && npm install http-server -g npm install hexo-cli -g && npm install hexo-deployer-git --save"
    o(action)
  ...
  if type == "2":
    ...
    hh()
    if key == "0":
      websitename = input("请输入网站名[最好是简短难忘的](实际上是文件夹名):")
